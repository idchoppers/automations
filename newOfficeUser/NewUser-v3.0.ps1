<#
    ...
    If user is of type (default, grp0, grp1)
...
$host.ui.RawUI.WindowTitle = "New User v3.0 - Status"

function NewDefaultUser() {
    $host.ui.RawUI.WindowTitle = "New User v3.0 - Working..."
    ...
    New-RemoteMailbox -Name "$($var_first.Text) $($var_last.Text)" -DisplayName "$($var_first.Text) $($var_last.Text)" -UserPrincipalName $upn -Password (ConvertTo-SecureString -String $password -AsPlainText -Force) -FirstName $var_first.Text -LastName $var_last.Text -SamAccountName $loginid
    ...
    $host.ui.RawUI.WindowTitle = "New User v3.0 - Done!"
...
function NewGRP2User() {
    ...
    Add-ADGroupMember -Identity "group2" -Members $loginid
